...
const ProjectInfo = ({ image, category, name, description, date, links, tags, onClick }) => {
  return (
    <div className="project-info">
      ...
      <div className="project-info__content">
        ...
        <div className="project-info__content--info">
          <h1 className="project-info__content--info__title">{name}</h1>
          ...
          {(tags && tags.length > 0) && <div className="project-info__content--info__tags">
            <ul>
              {tags.map((tag, i) => (
                <li key={`tag_${i + 1}`}>{tag}</li>
              ))}
            </ul>
          </div>}
          {(links && Object.keys(links).length > 0) &&<div className="project-info__content--info__links">
            <ul>
              {Object.keys(links).map((key, i) => (
                <li key={`link_${i + 1}`}><a href={links[key]} alt={key} target="_blank" rel="noopener noreferrer">
